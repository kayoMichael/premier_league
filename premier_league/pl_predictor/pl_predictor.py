...
class PLPredictor(BaseDataSetScrapper):
    ...
    def __init__(self):
        ...
        self.leagues = self.session.query(League).with_entities(League.name, League.up_to_date_season, League.up_to_date_match_week).all()
    ...
    def create_dataset(self, output_path: str):
        ...
        for game in games:
            home_stats = next((stats for stats in game.game_stats if stats.team_id == game.home_team_id), None)
            away_stats = next((stats for stats in game.game_stats if stats.team_id == game.away_team_id), None)
            ...
            for key, value in vars(home_stats).items():
                if not key.startswith('_') and key not in ['id', 'game_id', 'team_id', 'game', 'team']:
    ...
    def match_statistic(self, season, team):
        ...
        if team:
            team = (self.session.query(Team)
                    .options(
                        joinedload(Team.home_games),
                        joinedload(Team.away_games)
                    )
    ...
    def update_data_set(self):
        ...
        self.pages = self.scrape_and_process_all(self.urls, rate_limit=4, desc="Fetching Season Schedule")
        ...
        self.scrape_and_process_all(filtered_urls, rate_limit=4, desc="Fetching Match Details", process_func=self._process_data)

        # First get max season per league
        latest_seasons = (
            self.session.query(
                League.name,
                func.max(Game.season).label('max_season')
            )
            .join(Game)
        ...
        for index, page in enumerate(self.pages):
            ...
            if match:
                ...
                if season == leagues_dict[league][0]:
                    relevant_xpath = MATCHES.match_report_url(match_week=leagues_dict[league][1])

            all_urls.extend([f"https://fbref.com{url}" for url in page.xpath(relevant_xpath)])
        return all_urls

    @staticmethod
    def _wrap_result_with_url(result: ElementTree, url: str, **kwargs) -> Union[tuple, None]:
        """
    ...
    def _return_by_position_data(table):
        ...
        table['Position_Group'] = table['Primary_Position'].apply(
            lambda x: 'ST' if any(pos in str(x) for pos in ['FW', 'LW', 'RW', 'AM'])
            else 'MF' if any(pos in str(x) for pos in ['CM', 'DM', 'RM', 'LM'])
            else 'DF' if any(pos in str(x) for pos in ['CB', 'LB', 'RB'])
            else 'GK' if 'GK' in str(x)
            else 'Total')
    ...
    def _fetch_corresponding_data_from_table(self, summary_table: pd.DataFrame, passing_table: pd.DataFrame,
                                             defence_table: pd.DataFrame, possession_table: pd.DataFrame,
                                             miscellaneous_table: pd.DataFrame, goal_keeper_table: pd.DataFrame,
                                             possession: int) -> dict:
        """
        ...
        ST_summary, MF_summary, DF_summary, aggregate_summary = self._return_by_position_data(summary_table)
        data.update({
        ...
        ST_passing, MF_passing, DF_passing, aggregate_passing = self._return_by_position_data(passing_table)
        data.update({
        ...
        ST_defence, MF_defence, DF_defence, aggregate_defence = self._return_by_position_data(defence_table)
        data.update({
        ...
        ST_possession, MF_possession, DF_possession, aggregate_possession = self._return_by_position_data(
            possession_table)
        data.update({
        ...
        ST_miscellaneous, MF_miscellaneous, DF_miscellaneous, aggregate_miscellaneous = self._return_by_position_data(
            miscellaneous_table)
        data.update({
    ...
    def _process_data(self, page, **kwargs):
        ...
        try:
            ...
            try:
                ...
                match_week = int(re.search(r'Matchweek (\d+)', match_week_unrefined).group(1))
            except Exception:
            ...
            home_team_id = re.search(r'/squads/([^/]+)/', home_stats.xpath('.//a//@href')[0]).group(1)
            away_team_name = away_stats.xpath(MATCHES.A_TAG)[0]
            away_team_id = re.search(r'/squads/([^/]+)/', away_stats.xpath('.//a//@href')[0]).group(1)
            home_team_record = home_stats.xpath('./div')[2].text
            home_team_points = sum(x * y for x, y in zip((3, 1, 0), (int(n) for n in home_team_record.split('-'))))
            away_team_record = away_stats.xpath('./div')[2].text
            away_team_points = sum(x * y for x, y in zip((3, 1, 0), (int(n) for n in away_team_record.split('-'))))
            ...
            current_season = f"{current_year - 1}-{current_year}" if int(match_date.split("-")[1]) < 8 else f"{current_year}-{current_year + 1}"
            match_venue_time = match_info.xpath('./div')[0].xpath(MATCHES.GAME_VENUE_TIME)[0]
            match_time = datetime.strptime(f"{match_date} {match_venue_time}", "%Y-%m-%d %H:%M")
            home_goals = int(home_stats.xpath(MATCHES.GAME_GOALS)[0])
            away_goals = int(away_stats.xpath(MATCHES.GAME_GOALS)[0])

            tables = page.xpath('//table')
            home_possession = int(pd.read_html(etree.tostring(tables[2]), header=1)[0].iloc[0].iat[0].replace("%", ""))
            away_possession = int(pd.read_html(etree.tostring(tables[2]), header=1)[0].iloc[0].iat[1].replace("%", ""))
            ...
            team_data_one = self._fetch_corresponding_data_from_table(
                summary_table, passing_table, defence_table,
                possession_table, miscellaneous_table,
                goal_keeper_table, home_possession
            )
            ...
            team_data_two = self._fetch_corresponding_data_from_table(
                summary_table, passing_table, defence_table,
                possession_table, miscellaneous_table,
                goal_keeper_table, away_possession
            )
