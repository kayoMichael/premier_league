...
class TestTransferRoutes:
    ...
    @patch("premier_league.api.services.transfer_service.TransferService.get_all_current_teams")
    ...
    @patch("premier_league.api.services.transfer_service.TransferService.get_transfer_in_data")
    def test_get_transfer_in_data(self, mock_get_transfer_in, mock_transfer_data, client):
        ...
        mock_get_transfer_in.assert_called_with(team="Arsenal", season=None, league="Premier League")

        # Test with all parameters
        response = client.get("/transfers/in?team=Barcelona&season=2022-2023&league=La Liga")
        assert response.status_code == 200
        mock_get_transfer_in.assert_called_with(team="Barcelona", season="2022-2023", league="La Liga")

    @patch("premier_league.api.services.transfer_service.TransferService.get_transfer_out_data")
    def test_get_transfer_out_data(self, mock_get_transfer_out, mock_transfer_data, client):
        ...
        assert response.status_code == 200
        assert json.loads(response.data) == mock_transfer_data
        mock_get_transfer_out.assert_called_with(team="Arsenal", season=None, league="Premier League")

        # Test with all parameters
        response = client.get("/transfers/out?team=Barcelona&season=2022-2023&league=La Liga")
        assert response.status_code == 200
        mock_get_transfer_out.assert_called_with(team="Barcelona", season="2022-2023", league="La Liga")

    @patch("premier_league.api.services.transfer_service.TransferService.transfer_csv")
    def test_get_transfer_data_csv(self, mock_transfer_csv, create_temp_file, client, tmp_path):
        """Test GET /transfers/csv_file endpoint."""
        mock_transfer_csv.side_effect = lambda *args, **kwargs: (create_temp_file("test_transfers.csv", "Player,From,To,Fee\nMartin Odegaard,Real Madrid,Arsenal,£30m"), 200)
        ...
        response = client.get("/transfers/csv_file?team=Arsenal&filename=test_transfers&transfer_type=invalid")
        assert response.status_code == 400
        assert json.loads(response.data) == {"error": "Invalid type parameter"}

        # Test with required parameters (default transfer_type)
        response = client.get("/transfers/csv_file?team=Arsenal&filename=test_transfers")
        assert response.status_code == 200
        assert response.headers["Content-Type"] == "text/csv; charset=utf-8"
        assert response.headers["Content-Disposition"] == "attachment; filename=test_transfers.csv"
    ...
    def test_get_transfer_data_json(self, mock_transfer_json, create_temp_file, client, tmp_path):
        """Test GET /transfers/json_file endpoint."""
        mock_transfer_json.side_effect = lambda *args, **kwargs: (create_temp_file("test_transfers.json", '{"transfers": [{"Player": "Martin Odegaard", "From": "Real Madrid", "To": "Arsenal", "Fee": "£30m"}]}'), 200)
        ...
        # Invalid transfer_type parameter
        response = client.get("/transfers/json_file?team=Arsenal&filename=test_transfers&transfer_type=invalid")
        assert response.status_code == 400
        assert json.loads(response.data) == {"error": "Invalid type parameter"}

        # Test with required parameters (default transfer_type)
        response = client.get("/transfers/json_file?team=Arsenal&filename=test_transfers")
        assert response.status_code == 200
        assert response.headers["Content-Type"] == "application/json"
        assert response.headers["Content-Disposition"] == "attachment; filename=test_transfers.json"
