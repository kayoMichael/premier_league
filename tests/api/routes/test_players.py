...
class TestPlayersRoutes:
    ...
    def test_get_scorers(self, mock_get_goals, mock_player_data, client):
        ...
        assert response.status_code == 200
        mock_get_goals.assert_called_with(season='2022-2023', limit=None, league='Premier League')

        # Test with all parameters
        response = client.get('/players/goals?season=2022-2023&limit=5&league=Premier league')
        assert response.status_code == 200
        mock_get_goals.assert_called_with(season='2022-2023', limit=5, league='Premier league')
    ...
    def test_get_assists(self, mock_get_assists, mock_player_data, client):
        ...
        assert response.status_code == 200
        mock_get_assists.assert_called_with(season='2022-2023', limit=10, league='Premier League')

    @patch('premier_league.api.services.player_service.PlayerService.get_player_data_goals_csv')
    def test_get_scorers_csv(self, mock_get_csv, create_temp_file, client, tmp_path):
        """Test GET /players/goals/csv_file endpoint."""

        # Mock the services response
        mock_get_csv.side_effect = lambda *args, **kwargs: (create_temp_file("test_goals.csv", "name,team,goals\nHaaland,Man City,36\nKane,Spurs,30\n"), 200)

        # Test with required parameters
        response = client.get('/players/goals/csv_file?filename=test_goals')
        assert response.status_code == 200
        assert response.headers['Content-Type'] == 'text/csv; charset=utf-8'
        assert response.headers['Content-Disposition'] == 'attachment; filename=test_goals.csv'
    ...
    @patch('premier_league.api.services.player_service.PlayerService.get_player_data_assists_csv')
    ...
    def test_get_scorers_json(self, mock_get_json, mock_player_data, create_temp_file, client, tmp_path):
        ...
        assert response.headers['Content-Disposition'] == 'attachment; filename=test_goals.json'
    ...
    def test_get_assists_json(self, mock_get_json, mock_player_data, create_temp_file, client, tmp_path):
        ...
        assert response.status_code == 200
        assert response.headers['Content-Type'] == 'application/json'
        assert response.headers['Content-Disposition'] == 'attachment; filename=test_assists.json'
        ...
        # Test with invalid limit
        response = client.get('/players/assists/json_file?filename=test_assists&limit=xyz')
        assert response.status_code == 400
        assert json.loads(response.data) == {"error": "Limit must be a number"}
